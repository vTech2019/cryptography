...
static struct PART_TEST_VECTOR {
	size_t offset;
	uint8_t true_result[16];
};

static struct RC4_TEST_VECTORS {
	const char* key;
	struct PART_TEST_VECTOR vector[18];
};

static struct RC4_TEST_VECTORS test_vector[] =
{
		"0102030405",
...
size_t RC4Debug() {
	...
	unsigned char text[0x10ff] = { 0 };
	for (size_t i = 0; i < sizeof(test_vector) / sizeof(*test_vector); i++) {
		size_t keySize = strlen(test_vector[i].key) / 2;
		unsigned char* key = (unsigned char*)malloc(keySize);
		for (size_t j = 0; j < keySize; j++) {
			unsigned char tmp = 0;
			for (size_t k = 0; k < 2; k++) {
				tmp <<= 4;
				unsigned char sub_data = (unsigned char)test_vector[i].key[j * 2 + k] >= 'a' ? 'a' - 10 : '0';
				tmp ^= ((unsigned char)test_vector[i].key[j * 2 + k] - sub_data) & 0xf;
			}
			key[j] = tmp;
		}
		memset(text, 0, sizeof(text));
		RivestCipher4Encrypt(text, sizeof(text), key, keySize);
		for (size_t j = 0; j < sizeof(test_vector[i].vector) / sizeof(*test_vector[i].vector); j++) {
			size_t offset = test_vector[i].vector[j].offset;
			for (size_t k = 0; k < sizeof(test_vector[i].vector[j].true_result); k++) {
				if (text[offset + k] != test_vector[i].vector[j].true_result[k]) {
					printf("ERROR! %d %d %d\n", i, j, k);
					return 1;
				}
			}
